...
- (void) processStartOfPartWithHeader:(MultipartMessageHeader *)header
{
    ...
    myFileName = [[disposition.params objectForKey:@"filename"] lastPathComponent];
    
    if (myFileName==nil || [myFileName isEqualToString:@""])
    {
         return;
    }
    
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
   uploadFilePath = [[paths objectAtIndex:0] stringByAppendingPathComponent:myFileName];
    ...
    isUploading = YES;
    storeFile = [NSFileHandle fileHandleForUpdatingAtPath:uploadFilePath];
